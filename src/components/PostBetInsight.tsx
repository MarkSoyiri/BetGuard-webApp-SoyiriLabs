...
export function PostBetInsight({ open, slips, onClose }: PostBetInsightProps) {
  ...
  const { healthBonus, activeChallenges } = useChallenges();
    ...
    >
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-2 rounded-xl bg-slate-100/80 px-4 py-2.5 text-xs font-semibold text-slate-500 dark:bg-slate-800/80 dark:text-slate-300">
          ...
          Health score {snapshot.health.score + healthBonus} / 100 · {snapshot.health.level} risk · limits{' '}
          {limits.enabled ? 'on' : 'off'}
          {activeChallenges[0] && (
            <span className="ml-auto truncate text-[11px] text-primary-light">
              Next goal: {activeChallenges[0].challenge.title} ({activeChallenges[0].pct}%)
            </span>
          )}
        </div>
